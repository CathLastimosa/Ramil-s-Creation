...
import { formatTimeTo12Hour, getAllEventsForDay, getEventsForDay, getSpanningEventsForDay, sortEvents } from './utils';
                ...
                {weeks.map((week, weekIndex) => (
                    <div key={`week-${weekIndex}`} className="grid grid-cols-7 [&:last-child>*]:border-b-0">
                        {week.map((day, dayIndex) => {
                            ...
                            const blockedEvents = allEvents.filter((e) => e.id && typeof e.id === 'string' && e.id.startsWith('blocked-'));
                            ...
                            return (
                                <div
                                    key={day.toString()}
                                    className={`group border-r border-b border-border/70 last:border-r-0 data-outside-cell:bg-muted/25 data-outside-cell:text-muted-foreground/70 ${hasBlocked ? 'bg-gray-200' : ''}`}
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            {sortEvents(
                                                allDayEvents.filter((e) => !e.id || typeof e.id !== 'string' || !e.id.startsWith('blocked-')),
                                            ).map((event, index) => {
                                                    ...
                                                    >
                                                        <div className="space-y-2">
                                                            ...
                                                            <div className="space-y-1">
                                                                {sortEvents(
                                                                    allEvents.filter(
                                                                        (e) => !e.id || typeof e.id !== 'string' || !e.id.startsWith('blocked-'),
                                                                    ),
                                                                ).map((event) => {
                                                                    const eventStart = new Date(event.start);
                                                                    const eventEnd = new Date(event.end);
