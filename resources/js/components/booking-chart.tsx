...
export default function BookingChart({ bookingsData, period }: BookingChartProps) {
    const total = useMemo(() => bookingsData.reduce((sum, item) => sum + item.value, 0), [bookingsData]);

    const color = 'var(--color-accent2)';
    const gradientId = 'bookingsGradient';

    return (
        <div className="h-full space-y-4 p-4">
            <div className="flex items-center gap-2">
                <CalendarCheck2 className="size-5" style={{ color }} />
                <span className="text-base font-semibold text-red-950 dark:text-white">Total Bookings</span>
            </div>
