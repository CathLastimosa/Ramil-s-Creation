...
export function WeekView({ currentDate, events, onEventSelect, onEventCreate }: WeekViewProps) {
    ...
    const blockedEventsPerDay = useMemo(() => {
        return days.map((day) =>
            events.filter((e) => e.id && typeof e.id === 'string' && e.id.startsWith('blocked-') && isSameDay(new Date(e.start), day)),
        );
    }, [days, events]);
                    ...
                    <div className="grid grid-cols-8">
                        ...
                        {days.map((day, dayIndex) => {
                            ...
                            return (
                                <div
                                    key={day.toString()}
                                    className={`relative border-r border-border/70 p-1 last:border-r-0 ${hasBlocked ? 'bg-gray-200' : ''}`}
                                    data-today={isToday(day) || undefined}
                                >
                                    ...
                                    {hasBlocked && blockedEvents.length > 0 && (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                                            <div className="text-center text-sm font-semibold text-gray-800">{blockedEvents[0].title}</div>
                                            <div className="text-xs text-gray-700">
                                                {formatTimeTo12Hour(blockedEvents[0].start_time)}-{formatTimeTo12Hour(blockedEvents[0].end_time)}
                                            </div>
                                        </div>
                                    )}
                                </div>
            ...
            <div className="grid flex-1 grid-cols-8 overflow-hidden">
                ...
                {days.map((day, dayIndex) => {
                    ...
                    return (
                        <div
                            key={day.toString()}
                            className={`relative grid auto-cols-fr border-r border-border/70 last:border-r-0 ${hasBlocked ? 'bg-gray-200' : ''}`}
                            data-today={isToday(day) || undefined}
                        >
                            {/* Positioned events */}
